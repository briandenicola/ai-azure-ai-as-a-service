# run-apim-smoke-test.ps1
#
# Fires a test run for the 'apim-smoke-test' definition in Azure Load Testing.
# Hits APIM directly over the internal VNet path (no App Gateway) — fastest
# way to confirm the deployment is healthy after azd provision && azd deploy.
#
# Usage:
#   pwsh scripts/run-apim-smoke-test.ps1
#   pwsh scripts/run-apim-smoke-test.ps1 -Engines 2 -Description "Pre-release smoke"
#
# Prerequisites:
#   azd provision must have completed. configure-load-test.ps1 runs automatically
#   as a postprovision hook and creates the 'apim-smoke-test' definition.
#
# Exit codes:
...
$displayName = if ($Description) { $Description } else { "APIM smoke test $(Get-Date -Format 'yyyy-MM-dd HH:mm')" }

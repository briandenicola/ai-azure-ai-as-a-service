...
Also ensure `X-Model-Name` is in the `apimAppInsightsDiagnostics` frontend response headers list (Step 3c) so it flows through to `AppRequests.Properties["Response-Header-X-Model-Name"]` in Log Analytics.

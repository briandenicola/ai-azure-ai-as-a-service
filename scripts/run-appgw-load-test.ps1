...
<#
.SYNOPSIS
  Create (or update) the 'appgw-smoke-test' Azure Load Testing test and run it.
  Compares results against the baseline 'apim-smoke-test' (direct APIM, no AppGW).
...
$SILVER_KEY = (az rest --method POST `
    --uri "https://management.azure.com/subscriptions/$subId/resourceGroups/$ALT_RG/providers/Microsoft.ApiManagement/service/$APIM_NAME/subscriptions/silver-test/listSecrets?api-version=2022-08-01" `
    2>$null | ConvertFrom-Json).primaryKey
...
# Generated by run-appgw-load-test.ps1 — DO NOT COMMIT
APIM_HOSTNAME=$APPGW_FQDN
API_VERSION=2024-10-21
BRONZE_KEY=$BRONZE_KEY
SILVER_KEY=$SILVER_KEY

...
The `tests/run_azure_load_test*.ps1` scripts submit JMX files to the **Azure Load Testing** service (`lt-contoso-ai-dev1`), which runs JMeter in managed Azure agents. This is preferred because:
...
JMeter auto-reads `system.properties` from its working directory, so run all `jmeter` commands **from the `tests/` directory**, or pass the file explicitly with `-p tests/config/system.properties`.
...
jmeter -n -t tests/definitions/steady-state-test.jmx `
  -p tests/config/appgw-system.properties `

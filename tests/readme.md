...
|---|---|---|
| `apim-load-test.jmx` | Direct APIM (internal VNet — ALT only) | BRONZE_KEY, SILVER_KEY |
| `appgw-load-test.jmx` | App Gateway → APIM → Foundry | BRONZE_KEY, SILVER_KEY |
| `steady-state-test.jmx` | 1-hour steady state, all 4 LOBs | BRONZE_KEY, SILVER_KEY, SILVER_KEY_2, GOLD_KEY |
| `failover-load-test.jmx` | Circuit-breaker failover | SILVER_KEY |
| `multi-sub-failover-test.jmx` | Multi-subscription failover | BRONZE_KEY, SILVER_KEY |

## Normal usage — Azure Load Testing (preferred)

The `scripts/run-*.ps1` scripts submit JMX files to the **Azure Load Testing** service (`lt-contoso-ai-dev1`), which runs JMeter in managed Azure agents. This is preferred because:
...
The App Gateway uses a self-signed cert. `system.properties` in this directory already tells JMeter to skip certificate validation (`trustStore=NONE`), which is the simplest option for local testing:
...
JMeter auto-reads `system.properties` from its working directory, so run all `jmeter` commands **from the `tests/` directory**, or pass the file explicitly with `-p`.

Alternatively, if you want full cert validation, use `appgw-system.properties` + `appgw-truststore.p12` (generated by `scripts/create-appgw-cert.ps1`):

```powershell
# Cert-validating variant (run from repo root)
jmeter -n -t tests/steady-state-test.jmx `
  -p tests/appgw-system.properties `
  "-JAPIM_HOSTNAME=$APPGW_FQDN" ...
```

### Step 3 — Run a test

Run from the `tests/` directory so `system.properties` is picked up automatically.

**Steady-state (all 4 LOBs, 1 hour):**
```powershell
cd c:\gitrepos\ai-azure-ai-as-a-service\tests

jmeter -n -t steady-state-test.jmx `
  "-JAPIM_HOSTNAME=$APPGW_FQDN" `
...
```
Open `tests/html-report/index.html` in a browser when complete.

**App Gateway smoke (Bronze + Silver only):**
```powershell
jmeter -n -t appgw-load-test.jmx `
  "-JAPIM_HOSTNAME=$APPGW_FQDN" `
  "-JAPI_VERSION=2024-10-21" `
  "-JBRONZE_KEY=$BRONZE_KEY" `
  "-JSILVER_KEY=$SILVER_KEY" `
...
```powershell
jmeter -n -t failover-load-test.jmx `
  "-JAPIM_HOSTNAME=$APPGW_FQDN" `
  "-JAPI_VERSION=2024-10-21" `
  "-JSILVER_KEY=$SILVER_KEY" `
  "-Jsummariser.interval=10" `
...
Open `tests/html-report/index.html` in a browser — JMeter generates it automatically at the end of each run.
...
| File | Purpose |
|---|---|
| `system.properties` | JMeter system props — trust-all TLS (auto-read from working dir) |
| `appgw-system.properties` | JMeter system props — cert-validating TLS (uploaded to ALT alongside JMX) |
| `appgw-truststore.p12` | PKCS12 truststore for App Gateway self-signed cert (password: `changeit`) |
| `appgw-cert.cer` | App Gateway public cert (DER format) |
